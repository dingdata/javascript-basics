...
function keepSingleWords(words) {
  return words.filter(singleWordReturn => !singleWordReturn.includes(" "));
...
function filterUnderagedPeople(people, ageLimit) {
  return people.filter(person => person.age > ageLimit).map(adult => adult.name);
}
...
function filterEmailsByDomain(domain, ...emails) {
  console.log(emails);
  return emails.filter(email => email.includes(domain));
}
